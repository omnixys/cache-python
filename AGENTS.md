...
> Repository development instructions for **cache** (Shared Package).
> Stack: python-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys shared cache package (Redis/Valkey client, serialization, invalidation).

- Repository path: `packages/python/cache` (relative to the Omnixys root)
...
src/cache/ Redis/Valkey client + serialization + invalidation
...
Public API: cache client interfaces, serializers, invalidation contracts.
